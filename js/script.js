...
$.parseYouTubeData = function(result){
	...
	for (var i in items){
		...
		var thumb = '<span class="thumbnail_wrapper"><img src="'+ item.thumbnail.sqDefault +'" alt="'+ item.title +'" title="'+ item.title +'" /><time>'+ $.formatTime(item.duration) +'</time></span>';
		var item_link = '<a href="#movie_'+ item.id +'" title="'+ item.title +'">'+ thumb +'<strong>'+ item.title +'</strong><span class="author">'+ item.uploader +'</span><span class="viewCount">'+ item.viewCount +'</span><span class="date">'+ item_date +'</span></a>'
...
$.changeVideo = function(id){
	$.get('http://gdata.youtube.com/feeds/api/videos/'+ id +'?v=2&alt=jsonc', {}, function(result){
		...
		var code = '<div id=\"movie_' + video.id + '\" class=\"activemovie\"><h1>'+ video.title +'</h1><object width=\"745\" height=\"448\"><param name=\"wmode\" value=\"opaque\" /><param name=\"movie\" value=' + video.content[5] + ' /><param name=\"allowFullScreen\" value=\"true\" /><param name=\"allowscriptaccess\" value=\"always\" /><embed src=' + video.content[5] + ' type=\"application/x-shockwave-flash\" allowscriptaccess=\"always\" allowfullscreen=\"true\" width=\"745\" height=\"448\" wmode=\"opaque\"></embed></object></div>';
		$('#video').html(code);
		$('html, body').animate({
